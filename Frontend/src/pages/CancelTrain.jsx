...
export default function CancelTrain() {
  ...
  // Cancel alternative booking
  const handleCancelAlternativeBooking = async (id) => {
    if (!window.confirm("Are you sure you want to cancel this alternative booking?")) {
      return;
    }

    try {
      await axios.delete(`${BASE_URL}/api/alternativebookings/${id}`);
      alert("Alternative booking cancelled successfully.");
      fetchBookings();
    } catch (err) {
      console.error(err);
      alert("Error cancelling alternative booking.");
    }
  };
          ...
          {bookings.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {bookings.map((b) => (
                <div
                  key={b._id}
                  className="bg-white border border-gray-200 rounded-lg shadow p-6 hover:shadow-lg transition relative"
                >
                  ...
                  <button
                    className="absolute bottom-0 right-4 text-red-600 hover:text-red-800 font-semibold text-sm"
        ...
        <section>
          ...
          {alternativeBookings.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {alternativeBookings.map((a) => (
                <div
                  key={a._id}
                  className="bg-green-50 border border-green-200 rounded-lg shadow p-6 hover:shadow-lg transition relative"
                >
                  ...
                  <button
                    className="absolute bottom-0 right-4 text-red-600 font-semibold text-sm"
                    onClick={() => handleCancelAlternativeBooking(a._id)}
                  >
                    Cancel
                  </button>
                </div>
