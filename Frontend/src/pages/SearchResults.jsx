...
export default function SearchResults() {
  ...
  // Check if all trains are fully booked in all classes
  const allTrainsFull =
    searchResults.length > 0 &&
    searchResults.every((train) => {
      return (
        Number(train.sleeper_availability || 0) === 0 &&
        Number(train["1A_availability"] || 0) === 0 &&
        Number(train["2A_availability"] || 0) === 0 &&
        Number(train["3A_availability"] || 0) === 0
      );
    });
    ...
    <div className="flex">
      ...
      <div className="flex-1 md:px-8 pt-16 lg:ml-72 lg:pt-20 min-h-screen">
        <div className="max-w-4xl mx-auto p-4">
          ...
          {/* Alternative Trains Button */}
          {searchResults.length > 0 && allTrainsFull && (
            <button
              className="mb-5 inline-block bg-yellow-500 text-white px-4 py-2 rounded hover:bg-yellow-600 transition-colors"
              onClick={() => {
                navigate("/home/trains/searchtrains/searchresults/alternatives");
              }}
            >
              Show Alternative Trains
            </button>
          )}
